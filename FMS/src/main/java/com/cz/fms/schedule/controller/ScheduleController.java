...
public class ScheduleController {
	...
	public String addViewPOST(Schedule schedule, Model model) throws Exception {
		...
		if (schedule.getSchedule_deadline() == null) {// 마감을 안정했을 시
			schedule.setSchedule_deadline(schedule.getSchedule_date());// 데드라인을
																		// 일정날짜와
																		// 같게
																		// 설정해준다.
		}
	...
	/**
	 * 일정 전체보기 메소드로 /schedule/listAll로 GET요청이 들어왔을 경우 실행 value : List<Schedule>
	 * 
	 * @author "안광호"
	 */
	...
	/**
	 * 상세 일정 보기 메소드로 /schedule/detail_View로 GET요청이 들어왔을 경우 실행 paranname :
